 ...
 * @createTime : 2024/2/18 14:16
 * @Email : gumorming@163.com
 * @Description : 对RedisTemplate进一步封装
 */
...
public class RedisCache {
    ...
    public List<LoginUser> getLoginUsersByPrefix(String loginUserPrefix) {

        List<String> keys = getKeysByPrefix(loginUserPrefix);
        List<Object> values = redisTemplate.opsForValue().multiGet(keys);
        return (List<LoginUser>) (List) values;

    }

    public Map<String, String> getKeyValueMapByPrefix(String keyPrefix) {
        Map<String, String> keyValueMap = new HashMap<>();
        Set<String> keys = redisTemplate.keys(keyPrefix + "*");
        keys.stream()
                .map(key -> {
                    String userId = key.substring(RedisConstants.TOKEN_PREFIX.length());
                    keyValueMap.put(userId, getCacheObject(key));
                    return null;
                })
